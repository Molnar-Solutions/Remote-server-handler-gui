...
export class UserService {
    ...
    async signIn(user: UserLoginDto) {
        try {
            ...
            if (!foundUser) throw new Error("Nincs ilyen felhasználó vagy nincs aktiválva!");
    ...
    async signOut(token: string) {
        try {
            ...
            if (!foundUser) throw new BadRequestException("Hiba! A token nem érvényes!");
            ...
            await this.loggingService.log({
                user: "System", message: `Egy felhasználó kilépett: ${foundUser.email},
                időpont: ${moment(new Date()).format("YYYY.MM.DD HH:mm")}`, type: ESeverity.Information
            });
    ...
    async isLoggedIn(token: string) {
        try {
            ...
            if (!isLogged) throw new BadRequestException("Hiba! Nem vagy bejelentkezve!");
